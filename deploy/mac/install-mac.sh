...
#
# Jobs: com.eqr.web (dashboard + advisor API, 127.0.0.1:8801, KeepAlive),
...
[ -x "$ROOT/.venv/bin/eqr" ] || { echo "no venv at $ROOT/.venv — run: uv venv --python 3.13 .venv && uv pip install --python .venv/bin/python -e '.[dev]'" >&2; exit 1; }
